...
### Pitfall: officecli PPT design is ugly — use python-pptx for professional decks

officecli can create PPTX files but produces **ugly, unprofessional output** — flat backgrounds, default fonts, no card layouts, no visual hierarchy. For professional pitch decks, fundraising decks, or any presentation where design matters, **use python-pptx directly** with a proper design system (Dark Navy theme, card layouts, accent bars, typography hierarchy).

**When to use officecli for PPT:**
- Quick internal documents, data tables, simple reports
- When design quality doesn't matter

**When to use python-pptx for PPT:**
- Pitch decks, fundraising decks, client-facing presentations
- Any presentation where visual quality matters
- See `pitch-deck-creation` skill for full workflow and `templates/pitch-deck-dark-theme.py` template

### Pitfall: Install script puts SKILL.md in wrong directory

The auto-installer detects Hermes Agent and saves SKILL.md to `~/.hermes/skills/officecli/SKILL.md` (flat, no category). Hermes expects skills under a category subdirectory. After install, fix immediately:

```bash
mkdir -p ~/.hermes/skills/productivity/officecli
mv ~/.hermes/skills/officecli/SKILL.md ~/.hermes/skills/productivity/officecli/SKILL.md
rm -rf ~/.hermes/skills/officecli
```

### Pitfall: Font inconsistency when using python-pptx enhancement

When enhancing SlideSage-generated PPTX with python-pptx, fonts become inconsistent because:
- SlideSage uses its own font system
- python-pptx sets fonts per-run, not per-paragraph
- Chinese text needs CJK-capable fonts (PingFang SC, Noto Sans SC)

**Fix:** Always run a font standardization pass after any python-pptx enhancement:
```python
CN_FONT = "PingFang SC"
EN_FONT = "Georgia"
BODY_FONT = "Helvetica Neue"
# Set font on every run in every paragraph in every text frame
```

### Pitfall: business-proposal skill exists for pitch decks

For fundraising decks / 企划书 / BP, use the `business-proposal` skill FIRST. It has the complete workflow (research → interview → SlideSage → python-pptx enhancement). Don't start from scratch with officecli alone.

### Enforcement Rule

OfficeCLI is **mandatory** for all .docx/.xlsx/.pptx tasks. Do NOT use python-docx, openpyxl, or python-pptx. This is enforced via skills-first-execution-protocol Principle 11.

**EXCEPTION: Pitch decks / Business proposals** — Do NOT use officecli for creating investor-facing presentations. Use SlideSage + python-pptx instead (see `pitch-deck-creation` skill). Officecli produces flat designs with default fonts and no visual hierarchy — unsuitable for professional pitch decks.

For business proposals (企划书/BP/PPT), use `officecli load_skill pitch-deck` first.

### ⚠️ Visual Quality Limitation — PPTX looks "AI-generated"

officecli produces **functional but visually basic** PPTX that users describe as:
- "ui排版太差" (layout is poor)
- "ai味太浓了" (looks too AI-generated)

**For investor-facing presentations, use SlideSage instead:**
- `business-proposal-creation` skill has full SlideSage workflow
- SlideSage produces professional .pptx with 7 curated styles
- Storyboard-driven, action titles, QA checks
- GitHub: vedraut/slidesage

**Decision guide:**
| Scenario | Tool |
|----------|------|
| Investor pitch deck | **SlideSage** (see references/slidesage-workflow.md) |
| High-end brand proposal | **SlideSage** (minimalist-luxury style) |
| Corporate internal report | officecli |
| Quick functional deck | officecli |
| Document editing (Word/Excel) | officecli |

### SlideSage Workflow (Professional PPTX)

For investor-facing presentations, use SlideSage (vedraut/slidesage):
...
# 2. Create storyboard.json (see references/slidesage-workflow.md)
...
# 5. QA check (must pass all)
node scripts/qa-report.mjs --storyboard storyboard.json --deck output.pptx
```

**Available styles:** `futuristic-tech` (dark), `corporate-bright` (light), `minimalist-luxury` (white/beige), `japanese-editorial`, `soft-clay-3d`, `modern-illustration`, `hand-drawn-editorial`

**User preference:** 阿戴 prefers **white/beige** (minimalist-luxury) over dark themes. Use `minimalist-luxury` as default.

**阿戴 UI design rules (CRITICAL):**
- "不要乱加色彩" — Do NOT add random colors. Keep monochromatic, use only necessary accent colors
- "先补全再改" — Completeness BEFORE polish. Finish all content, then optimize design
- "不要双栏布局，分两页写" — No two-column layouts for team/content. Use separate pages instead
- "注意排版注意美感注意ui注意交互动画" — Layout, aesthetics, UI, and animations all matter
- "不要红绿分栏" — Avoid red/green color-coded columns. Use neutral tag styles instead
- "先说明情况再修改" — Explain before making changes

### python-pptx Post-Processing

After SlideSage generates the PPTX, use python-pptx to fix fonts and add custom styling:
...
**Critical font fix for Chinese content:**
```python
from pptx import Presentation
prs = Presentation("output.pptx")
CN_FONT = "PingFang SC"  # Must use for Chinese text
...
See `references/slidesage-workflow.md` for full workflow and `references/pptx-post-processing.py` for the complete enhancement script.
...
## PPTX Template Filling Workflow

When given a blank template + reference filled version + source document:

1. **Confirm which file to modify** — Ask the user explicitly. The "template" is the file to edit; the "reference" is read-only guidance. Never assume.
2. **Scan both files**: `officecli view <template> text` vs `officecli view <reference> text` to identify empty vs filled sections.
3. **Map shape IDs**: Use `officecli query <file> 'shape:contains("SectionTitle")'` to find exact shape IDs for each empty section.
4. **Fill systematically**: Use `set --find --replace` on existing shapes for placeholder text; use `add --type textbox` when a section has a header shape but no body shape below it.
5. **Verify with screenshot**: `officecli view <file> screenshot -o /tmp/check.png` then `vision_analyze` to check for overlaps and containment.
6. **Check vertical space** before adding new text boxes — measure the gap between sections using shape y-coordinates. If < 1cm, don't add a new box; merge content into the existing shape above instead.

**Key pattern — filling empty placeholder text:**
```bash
# Shape has "Aim:\n" (empty after label). Fill it:
officecli set template.pptx '/slide[1]/shape[@id=13]' --find "Aim:" --replace "Aim:
Content goes here."
```

**Key pattern — adding body text below a header-only shape:**
```bash
# Header shape exists but no body shape. Create new textbox at the right position:
officecli add template.pptx '/slide[1]' --type textbox \
  --prop text="Body content" \
  --prop x=11165772emu --prop y=20854157emu \
  --prop width=9272979emu --prop height=2781838emu \
  --prop font.latin="Times New Roman" --prop size=32pt --prop color=text1
```
...
| `fill.transparency` on shapes | Use `opacity` (0.0-1.0) instead. Requires `fill` to be set first |
| `alignment` on shapes | Not a valid prop for shapes |
| `w`/`h` on pictures | Use `width` and `height` (e.g. `--prop width=8cm --prop height=4cm`) |
| Design-heavy pitch decks in PPTX | For investor decks/BP with rich visuals, HTML (frontend-slides skill) produces far better results than officecli PPTX. Use PPTX only when format is strictly required |
| `--prop fill.transparency=100%` on shapes | Use `--prop opacity=0.0` (range 0.0-1.0) or `--prop fill=none` for fully transparent background |
| Increasing height of text boxes to fix overflow | **NEVER blindly increase height.** Adjacent boxes overlap because positions are fixed. Instead: keep original height, shorten text, or reposition boxes. Always verify with `screenshot` after any height change. |
| `\n` in `--prop text="..."` | Creates **separate XML paragraphs**, each with own spacing. For poster text boxes this adds unwanted gaps. Use `--find`/`--replace` to append within existing paragraph structure instead. |
| Multiple `--find`/`--replace` on same shape | Can produce **duplicated content** if regex doesn't match and you retry with different pattern. Always check with `get --depth 1` after each replace to verify no duplication. |
| File locked by resident for external editing | After `officecli open` or auto-resident starts, the file is locked. Run `officecli close <file>` **before** telling user to open in WPS/PowerPoint. `osascript -e 'tell application "Microsoft PowerPoint" to quit'` if they already have it open. |
| `--prop alignment=center` on shapes | Not a valid prop. Center-align text via x/y positioning or use `--prop textAlign=center` if available |
| `--prop w=8cm --prop h=8cm` on pictures | Pictures use `width` and `height`, NOT `w`/`h`. Use `--prop width=8cm --prop height=4cm` |
| `--prop w=14cm --prop h=2cm` on shapes | Shapes also use `width`/`height`. Use `--prop width=14cm --prop height=2cm` |
| Modifying the wrong template file | Always confirm with user which file to modify. "Template" = file to edit; "reference/example" = read-only. Ask before acting. |
| Adding text box without checking space | Measure gap between section headers using shape y+height. If gap < 1cm, merge into existing shape with `--find --replace` instead of creating a new textbox that will overlap. |
| Ignoring user's "先提问" instruction | When the user says "ask first if unclear", DO NOT proceed with assumptions. Clarify file targets, content sources, and what to preserve before editing. |
...
- `references/business-proposal-standards.md` — 融资型企划书（BP）完整参考标准：Sequoia/YC框架、排版四原则、每页内容标准、配色/字体/图表规范、投资人阅读习惯。做BP前必读。
- `references/business-proposal-workflow.md` — 企划书制作工作流：资料收集清单、市场调研方法、PPT制作流程、设计规范。
- `references/slidesage-workflow.md` — SlideSage professional PPT generation workflow.
- `references/poster-template-workflow.md` — PPTX poster/template filling workflow: blank template + reference + source doc → fill empty sections systematically.
- `references/nextjs-pitfalls.md` — Next.js 14 App Router critical pitfalls: `<head>` tag, `.next` corruption, multiple dev servers, floating point indices.
- `references/poster-template-filling.md` — Workflow for filling empty poster templates from a report/thesis using a filled reference poster.
- `references/word-form-filling.md` — Workflow for filling existing .docx form templates (university forms, applications, government forms) using officecli batch with paraId addressing.
- `references/word-form-filling.md` — Complete workflow for filling Word forms (mitigating circumstances, applications, etc.) using officecli batch operations with paraId paths.
- Run `officecli load_skill pitch-deck` for PPTX-specific fundraising deck rules.
